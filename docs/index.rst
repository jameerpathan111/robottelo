...
$ export ROBOTTELO_JIRA_API_KEY=sdfsdg654g8df4gdf6g4df8g468dfg
...
.. code-block:: yaml
    ...
    JIRA:
        ...
        API_KEY: '@format {this.vault_jr_api_key}'

The ``settings.jira.api_key`` should now return the Jira API key read from Vault store.

The secret is saved with key ``vault_jr_api_key`` in vault store in VAULT_MOUNT_POINT_FOR_DYNACONF/VAULT_PATH_FOR_DYNACONF path.
